...
SL_WEAK void app_process_action(void)
{
  ...
  if (!was_pressed && is_pressed) {
      uint8_t value[] = "hey there, central.";
      sl_bt_gatt_server_send_indication(connection_handle, characteristic_handle, sizeof(value), value);
      uint8_t data[] = "this is data generated by the peripheral device (GATT server) and is meant to be very, very, very long in order to test L2CAP fragmentation and defragmentation";
      fragment_and_send_l2cap_data(connection_handle, cid, data, sizeof(data), max_pdu);
  }
